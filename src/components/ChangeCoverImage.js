...
const ProfilePhotoUpload = () => {
  ...
  const handleFileUpload = async () => {
    ...
    if (selectedFile) {
      ...
      formData.append("cover_image", blob, "photo.jpg");
    }

    try {
      const response = await fetch(
        "https://social-network-backend.sharpbody-p.com/public/api/update-cover-photo",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setImageSrc(null)
        setSelectedFile(null)
        toast.success("Cover Photo Successfully Update!", {
          position: "top-right",
          autoClose: 5000,
